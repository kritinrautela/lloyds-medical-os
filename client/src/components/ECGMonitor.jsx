...
export default function ECGMonitor({ bpm = 74, spo2 = 98, bp = '120/80' }) {
  ...
  return (
    <div className="hud-panel p-4 flex flex-col md:flex-row items-center justify-between gap-4 overflow-hidden relative border border-white/10 shadow-2xl">
      ...
      <div className="flex items-center gap-5 shrink-0 z-10">
        <div className="flex items-center gap-3">
          <div className="relative p-2 rounded-2xl bg-red-950/40 border border-red-500/30 shadow-lg shadow-red-950/50">
            <Heart className="w-6 h-6 text-red-500 fill-red-500 animate-heartbeat" />
            <span className="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-emerald-400 animate-ping" />
          </div>
          <div>
            <div className="flex items-baseline gap-1.5">
              <span className="text-2xl font-black font-mono text-white tracking-tight leading-none">{currentBpm}</span>
              <span className="text-[10px] font-mono font-bold text-slate-400 uppercase">BPM</span>
            </div>
            <p className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider flex items-center gap-1 mt-1">
        ...
        <div className="hidden sm:flex items-center gap-4 border-l border-white/10 pl-5">
          <div className="space-y-0.5">
            <div className="flex items-baseline gap-1">
              <span className="text-lg font-black font-mono text-cyan-400 leading-none">{spo2}%</span>
              <span className="text-[9px] text-slate-400 uppercase font-mono font-semibold">SpO2</span>
            </div>
            <p className="text-[9px] text-slate-400 uppercase tracking-wider font-semibold">Pulse Oximetry</p>
          </div>

          <div className="space-y-0.5 border-l border-white/10 pl-4">
            <div className="flex items-baseline gap-1">
              <span className="text-lg font-black font-mono text-purple-400 leading-none">{bp}</span>
              <span className="text-[9px] text-slate-400 uppercase font-mono font-semibold">mmHg</span>
            </div>
            <p className="text-[9px] text-slate-400 uppercase tracking-wider font-semibold">Blood Pressure</p>
          </div>
        </div>
      </div>

      {/* Real-time Oscilloscope Canvas with CRT Bezel */}
      <div className="flex-1 w-full h-16 relative flex items-center justify-center overflow-hidden rounded-xl bg-[#02050E] border border-emerald-500/25 shadow-inner scanline-overlay">
        <canvas
          ref={canvasRef}
          width={600}
          height={64}
        ...
        <div className="absolute top-1.5 left-2.5 pointer-events-none flex items-center gap-3">
          ...
          <span className="hidden lg:inline text-[9px] font-mono text-slate-500">GAIN: 1.0X</span>
        </div>
      ...
      <div className="flex items-center gap-2 shrink-0 z-10">
        <button
          onClick={toggleSound}
          className={`flex items-center gap-2 px-3.5 py-2 rounded-xl border text-xs font-bold transition-all active:scale-95 shadow-lg ${
            audioActive
              ? 'bg-emerald-950/70 border-emerald-500/80 text-emerald-300 shadow-emerald-950/60 glow-emerald'
              : 'bg-black/30 border-white/10 text-slate-400 hover:text-white hover:border-white/20'
          }`}
          title="Toggle Hospital Audio Chimes"
        >
          {audioActive ? <Volume2 className="w-4 h-4 text-emerald-400 animate-pulse" /> : <VolumeX className="w-4 h-4" />}
